...
const VoteButtons = ({
    ...
    id: string;
    upvotes: Models.DocumentList<VoteDoc>;
    downvotes: Models.DocumentList<VoteDoc>;
    className?: string;
}) => {
    const [votedDocument, setVotedDocument] = React.useState<VoteDoc | null>(); // undefined means not fetched yet
    ...
    React.useEffect(() => {
        (async () => {
            if (user) {
                ...
                setVotedDocument(() => response.documents[0] as unknown as VoteDoc | null);
            }
        })();
    }, [user, id, type]);
